...
object Config {
  ...
  val nycTrafficSpeedUrl =
            "http://real2.nyctmc.org/nyc-links-cams/LinkSpeedQuery.txt"

  val finalWekaSerInstsFNameTrafficSpeeds = "New_York_City_Link_Speed.bsi"

  /* This is the latest Traffic Volume Counts per each segment of each street
  ...
  val nycTrafficVolumeCount =
    "https://data.cityofnewyork.us/api/views/p424-amsu/rows.csv?accessType=DOWNLOAD"

  val finalWekaSerInstsFNameTraffVolume =
    "New_York_City_Traffic_Volume_Count.bsi"


  /* The way to join both URLs, the Real-Time Link (Traffic) Speed -which uses
   * polygonal coordinates of sub-sections in the New York City- and the
   * Traffic Volume Counts -which uses segments IDs of each part of streets in
   * NYC- is necessary to employ the Department of City Planning's LION Single
   * Line Street Base Map,
   *
   *     https://data.cityofnewyork.us/City-Government/LION/2v4z-66xt
   *
   *     http://www.nyc.gov/html/dcp/download/bytes/nyc_lion15b.zip
   *
   * which gives the polygonal coordinates of each segment ID, and then, with
   * this polygonal coordinates, to do a polygon intersection between both
   * polygons (Traffic Volume Counts' versus Real-Time Link(Traffic) Speed's)
   * using the GeoScript/GeoTools/JTS libraries.
   *
   ...
   * GeoDataBase. We use this other project for the download and the ETL of
   * the LION GeoDB, since it can be done only once (while the NYC Department
   * of City Planning doesn't update the LION GeoDB, which is done only
  ...
  val nycLionMultiLineStreetMapFName =
   ...
   * crossed from extreme to extreme in 15 minutes, at this optimum sustained
   * speed. (Higher speeds than this are ideal and better, of course, but more
   * costly.)
   *
  ...
  val nycTargetOptimumTraffSpeed: Double = 80.0

  object parserOpenDataCsv {

    val csvHeadersNycLinkSpeedUrl = Array(
                                          "Id",
                                          "Speed",
                                          "TravelTime",
                                          "Status",
    ...
    val optionsWekaCsv2SerInstLinkSpeed = Array(
                           "-D", "5", "-format", "M/d/yyyy HH:mm:ss",
                           "-S", "7-9"
                      )

    val intermCleanCsvFNameTrafficSpeeds = "New_York_City_Link_Speed.csv"

    val csvHeadersNycTraffVolumeCountsUrl = Array(
                                                  "ID",
                                                  "Segment ID",
                                                  "Roadway Name",
                                                  "From",
    ...
    val optionsWekaCsv2SerInstTraffVolumeCnt = Array(
                                                     "-D", "7",
                                                     "-format", "M/d/yyyy"
                                                    )

    val intermCleanCsvFNameTraffVolume =
