    ...
    for (f <- layer.features.toArray()) {
      ...
      breakable {
        for (constr <- filtering_geoms) { 
          ...
          if (!this_feature_is_selected && constr.polyg != null) {

            /*
             * Try if this feature's geometry is contained in the
             * constraint's polygon.
             * The most general -and optimum- way to handle this is to use:
             *
             *   IntersectionMatrix  filtering_geom.relate(feat_geom)
             *
            ...
            this_feature_is_selected = constr.polyg.contains(feat_geom)
          }
