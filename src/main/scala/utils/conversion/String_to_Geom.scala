...
object String_to_Geom {
    ...
    def convert_lat_long(in_s: String): jts.geom.Geometry =
    {
     ...
     * and next pairs by a comma ",", in to Geometry.
     *
     * NOTE: The difference between convert_lat_long(string) and
     *                              convert_long_lat(string)
    ...
    def convert_long_lat(in_s: String): jts.geom.Geometry =
    {
          val coords = catching(classOf[java.lang.RuntimeException]) opt
                         in_s.split("\\s+").map {
                                    case point_str: String => {
    ...
    def apply(in_s: String, LION_order: Boolean = true): jts.geom.Geometry =
          {
               if (LION_order) convert_long_lat(in_s)
                          else convert_lat_long(in_s)
          }
